...
Third retry of the dual-domain deploy after clearing the stale cross-region SSM
export. Cert stack came back from `UPDATE_ROLLBACK_FAILED` via
`continue-update-rollback --resources-to-skip ExportsWriteruswest209BD44F0A7CF058B`,
the orphan SSM parameter
`/cdk/exports/HabitAgility/HabitAgilityCertuseast1RefCert5C9FAEC174FEF386` was
manually deleted from us-west-2, and the next deploy creates a fresh export
instead of triggering the anti-drift "Some exports have changed!" failure. No
application or CDK code changes — recovery only.

## [0.11.2] - 2026-05-19

Retry of the v0.11.1 dual-domain deploy after the cross-region ExportsWriter
rejected the cert ARN replacement. No code changes — re-attempt only after the
cert stack rollback was unstuck.
...
- **`habitagility.com` registered** via Route 53 Domains (~$13/yr, auto-renew on).
  Hosted zone `Z01841113N4874957M94C` auto-created with NS + SOA records.
  CloudFront now serves three SANs: `habitagility.com` (apex, primary),
  `www.habitagility.com` (reflex), and `ght.vexom.io` (kept alive so old
  bookmarks still work). ACM cert validates across both hosted zones via
  `CertificateValidation.fromDnsMultiZone`. Three A-records (apex, www, legacy)
  alias the same CloudFront distribution.
